...
class TcershoufangSpider(RedisSpider):
    ...
    def parse(self, response):
        ...
        for provinceName in provinceList:
            for cityName in cityList[provinceName]: #http://hz.58.com/shangpu/
                #个人出租 二手房 求租 短租
                ...
                yield Request(cityAddressC, callback=self.C_Region, errback=self.errback_httpbin)
                yield Request(cityAddressE, callback=self.E_Region, errback=self.errback_httpbin)
                yield Request(cityAddressQ, callback=self.Q_List, errback=self.errback_httpbin)
    ...
    def C_Region(self, response):
        urls = response.xpath('//dl[contains(@class,"secitem secitem_fist")]//@href')[1:]
        for url in urls.extract():
            yield Request(parse.urljoin(response.url, url), callback=self.C_subRegion, errback=self.errback_httpbin)
    
    #二手房
    def E_Region(self, response):
        urls = response.xpath('//div[@class="filter-wrap"]/dl[1]//@href')[1:]
        for url in urls.extract():
            yield Request(parse.urljoin(response.url, url), callback=self.E_subRegion, errback=self.errback_httpbin)    
    
    #短租
    def D_Region(self, response):
        urls = response.xpath('//div[@class="relative"]/dl[@class="secitem"][1]/dd//@href')[1:]
        for url in urls.extract():
            yield Request(parse.urljoin(response.url, url), callback=self.D_subRegion, errback=self.errback_httpbin)    
    
    #写字楼
    def X_Region(self, response):
        urls = response.xpath('//dl[@class="secitem"][1]/dd//@href')[1:]
        for url in urls.extract():
            yield Request(parse.urljoin(response.url, url), callback=self.X_subRegion, errback=self.errback_httpbin)    
    
    #商铺 厂房
    def S_Region(self, response):
        urls = response.xpath('//div[@class="filter-wrap"]/dl[1]//@href')[1:]
        for url in urls.extract():
            yield Request(parse.urljoin(response.url, url), callback=self.S_subRegion, errback=self.errback_httpbin)    
    ...
    def C_subRegion(self, response):
        urlsmain = response.xpath('//dl[contains(@class,"secitem secitem_fist")]//@href')[1:]
        urlsSub = response.xpath('//div[contains(@class,"arealist")]//@href')
        if urlsSub:
            for url in urlsSub.extract():
                yield Request(parse.urljoin(response.url, url), callback=self.C_List, errback=self.errback_httpbin)
        else:
            for url in urlsmain.extract():
                yield Request(parse.urljoin(response.url, url), callback=self.C_List, errback=self.errback_httpbin)
    #二手房 E 分有无地铁
    def E_subRegion(self, response):
        ...
        if urlsNoSubway:
            for url in urlsNoSubway.extract():
                yield Request(parse.urljoin(response.url, url), callback=self.E_List, errback=self.errback_httpbin)
        elif urlsWithSubway:
            for url in urlsWithSubway.extract():
                yield Request(parse.urljoin(response.url, url), callback=self.E_List, errback=self.errback_httpbin)
        else:
            yield print('该区域没有商圈', callback=self.E_List)

    #短租 D
    def D_subRegion(self, response):
        urls = response.xpath('//div[@class="subarea"]//@href')
        if urls:
            for url in urls.extract():
                yield Request(parse.urljoin(response.url, url), callback=self.D_List, errback=self.errback_httpbin)
        else:
            yield print('该区域没有商圈', callback=self.D_List)
    ...
    def C_List(self, response):
        ...
        if next_url:
            yield Request(next_url, callback=self.C_List, errback=self.errback_httpbin)
        
    #二手房 商铺 写字楼 
    def E_List(self, response):
        r = redis.StrictRedis(host='106.75.166.130', port=6379, db=0, password='v5e7r8o4n4i9c0a9')
        ...
        if next_url:
            yield Request(next_url, callback=self.E_List, errback=self.errback_httpbin)
    
    #短租 D 
    def D_List(self, response):
        r = redis.StrictRedis(host='106.75.166.130', port=6379, db=0, password='v5e7r8o4n4i9c0a9')
        ...
        for url in urls:
            r.rpush('58House:All', url)
        if next_url:
            yield Request(next_url, callback=self.D_List, errback=self.errback_httpbin)
            
    #个人求租 Q 
    def Q_List(self, response):
        r = redis.StrictRedis(host='106.75.166.130', port=6379, db=0, password='v5e7r8o4n4i9c0a9')
        ...
        for url in urls:
            r.rpush('58House:All', url)
        if next_url:
            yield Request(next_url, callback=self.Q_List, errback=self.errback_httpbin)
    ...
    def errback_httpbin(self, failure):
        ...
        if failure.check(HttpError):
            ...
            self.logger.error('出现HttpError on %s', response.url)

        elif failure.check(DNSLookupError):
            # this is the original request
            request = failure.request
            self.logger.error('出现DNSLookupError on %s', request.url)

        elif failure.check(TimeoutError, TCPTimedOutError):
            request = failure.request
            self.logger.error('出现TimeoutError on %s', request.url)
